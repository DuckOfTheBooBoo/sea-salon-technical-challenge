...
import { ref, type Ref, computed, watch, onMounted, reactive } from "vue";
...
const modelValue = reactive<string[]>([]);
const open: Ref<boolean> = ref(false);
const searchTerm: Ref<string> = ref("");

const filteredServices = computed(() =>
  services.value.filter((i) => !modelValue.includes(i.service_name))
);

watch(modelValue, () => {
  emit("update:services", modelValue);
})
